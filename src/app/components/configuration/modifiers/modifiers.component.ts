...
export class ModifiersComponent {
  ...
  openDrawer(): void {
    this.visible = true;
    this.drawerTitle = 'New Modifier';
  }

  openEditDrawer(data: any): void {
    this.visible = true;
    this.isUpdating = true;
    this.drawerTitle = 'Edit Modifier';
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.isDataLoading = true;
        this.modifiersService.delete(id)
          .pipe(finalize(() => {
            this.isDataLoading = false;
          }))
          .subscribe({
            next: () => {
              this.msgService.success('Modifier deleted successfully');
  ...
  submit() {
    ...
    this.modifiersService.create(this.form.value)
      ...
      .subscribe({
        next: () => {
          this.msgService.success('Modifier created successfully');
          this.getInitData();
          this.closeDrawer();
        },
        error: (err) => {
    ...
    this.modifiersService.get({}, null, null, true)
      ...
      .subscribe({
        next: (res: any) => {
          ...
          const filteredData = res.map((modifier: any) =>
            selectedColumns.reduce((obj: Record<string, any>, key) => {
              if (key === 'active') {
                obj[headers[key]] = modifier[key] ? 'Active' : 'Inactive';
              } else if (key === 'created') {
                const date = new Date(modifier[key]);
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, 'Modifiers');
          ...
          link.setAttribute('download', 'Modifiers.xlsx');
